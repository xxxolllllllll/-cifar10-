...
model = resnet18_opt(num_classes=n_class) # 得到预训练模型
# model.conv1 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
# model.fc = torch.nn.Linear(512, n_class) # 将最后的全连接层修改
# 载入权重
model.load_state_dict(torch.load('checkpoint/ckpt_opt_SGDM_lr0.1_epo200_mom0.9.pt'))
...
print(f"Acc: {acc:.2f}% | Precision: {precision:.4f} | Recall: {recall:.4f} | F1: {f1:.4f}")
